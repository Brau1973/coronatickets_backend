...
public class PnlDatosPaquete extends JInternalFrame {
	...
	public PnlDatosPaquete() {
		...
		panelPaquete.setBounds(X_LABEL, Y_DIST, 680, 230);
		...
		panelEsp.setBounds(X_LABEL, Y_DIST * 9, 680, 200);
    ...
    public void cargarPanel(PaqueteEspectaculos p){
		...
		for (int i = 0; i < p.getEspectaculos().size(); i++) {
			...
			Object[] data = { esp.getNombre(), esp.getArtista().getNombre(), esp.getPlataforma().getNombre(), esp.getDescripcion() };
			tm.addRow(data);
		}
    }
