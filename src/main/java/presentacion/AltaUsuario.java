	 ...
	 if(e.getSource() == btnAceptar){
	     if(rbtnEspectador.isSelected()){
		  if(checkFormulario()&&modificarDatos()){
		      try{
			   ...
			   DtUsuario dte = new DtEspectador(strNickname, strNombre, strApellido, strEmail, dateFechaNac, strContrasenia);
	     ...
	     if(rbtnArtista.isSelected()){
		  if(checkFormulario2()&&modificarDatos()){
		      try{
			   // falta mirar chequeo de fecha
			   DtUsuario dta = new DtArtista(strNickname, strNombre, strApellido, strEmail, dateFechaNac, strContrasenia, strDescripcion, strBiografia, strLink);
